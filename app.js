...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Login Confirm Button with OTP Verification
  if (el.confirmLoginBtn) {
    el.confirmLoginBtn.addEventListener('click', async () => {
      ...
      // If OTP Section is open or email provided, verify OTP with server
      if (email && otpSection && !otpSection.classList.contains('hidden')) {
        if (!otp || otp.length !== 6) {
          if (authStatusMessage) {
            authStatusMessage.className = 'qb-status-msg error';
            authStatusMessage.textContent = '❌ Please enter the 6-digit OTP code received.';
            authStatusMessage.classList.remove('hidden');
          }
          return;
        }
        ...
        try {
          ...
          if (authStatusMessage) {
            authStatusMessage.className = 'qb-status-msg error';
            authStatusMessage.textContent = `❌ Verification Error: ${err.message}`;
            authStatusMessage.classList.remove('hidden');
          }
        }
      } else {
        // Direct confirmation if returning user
        updateProfileHud();
        if (authLoginPage) authLoginPage.classList.add('hidden');
        if (el.loginModal) el.loginModal.classList.add('hidden');
